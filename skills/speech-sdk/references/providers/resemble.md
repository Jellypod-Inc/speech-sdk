...
`/synthesize` always returns `audio_timestamps` (no opt-in flag), so when `timestamps: true` is set, the SDK aggregates Resemble's grapheme-level alignment (`graph_chars[]` + `graph_times[][start, end]`, in seconds) into the SDK's word list. Mirrors the ElevenLabs aggregator: split on whitespace, keep punctuation attached to the adjacent word.
...
The streaming `/stream` endpoint is bytes-only — `timestamps: true` on a streamed call falls back to the default Whisper STT pass.
