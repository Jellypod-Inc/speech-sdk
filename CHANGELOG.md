...
- **ElevenLabs `/with-timestamps` responses missing `audio_base64` fall back to plain TTS.** `eleven_v3` answers 200 with character alignment and no audio often enough that retrying the same `/with-timestamps` request still fails. When `includeTimestamps` is set and `audio_base64` is absent, the provider issues one `/v1/text-to-speech/{voice}` request with the same body and query. Timestamps come from that partial payload via `resolveElevenLabsTimestamps` when alignment is present. With no usable alignment the audio is returned without timestamps: `generate()` has no `TimestampProvider`, and `/v1/forced-alignment` stays on the caller's aligner (or the estimated floor) once native timings are missing or fail validation. Those timings describe the dropped synthesis, so a duration mismatch against the fallback audio fails native validation instead of being applied. An empty plain response still throws the existing `NoSpeechGeneratedError` (`provider_empty_response`, timestamps `request-id`, alignment presence). A non-2xx from the plain request surfaces as that request's provider error.
